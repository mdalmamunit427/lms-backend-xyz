...
export const reorderChapterItemsWithConflictResolution = async (
    ...
    const [lectures, quizzes] = await Promise.all([
        ...
        ).sort({ order: 1, createdAt: 1 }).session(session).hint({ chapter: 1, order: 1 }),
 ...
 * @param session - MongoDB session for transaction support
 */
export const reorderChaptersAndLecturesBulkOptimized = async (
    courseId: string,
    orderList: { chapterId: string; order: number; lectures?: { lectureId: string; order: number }[] }[],
    session: any
): Promise<void> => {
    ...
    const [allLectures, allQuizzes] = await Promise.all([
        Lecture.find(
            ...
            { _id: 1, chapter: 1, order: 1, createdAt: 1 }
        ).sort({ chapter: 1, order: 1, createdAt: 1 }).session(session),
    ...
    const lecturesByChapter = new Map<string, any[]>();
    const quizzesByChapter = new Map<string, any[]>();

    allLectures.forEach(lecture => {
        const chapterId = (lecture.chapter as any).toString();
    ...
    allQuizzes.forEach(quiz => {
        const chapterId = (quiz.chapter as any).toString();
    ...
    const chapterBulkOps: any[] = [];
    const lectureBulkOps: any[] = [];
    const quizBulkOps: any[] = [];
    ...
    const chapterFinalOrdering = new Map<string, number>();
    const placedChapters = new Set<string>();
    ...
    if (chapterBulkOps.length > 0) {
        bulkPromises.push(
            Chapter.bulkWrite(chapterBulkOps, { 
                session, 
                ordered: false,
                writeConcern: { w: 1 }
            })
    ...
    if (lectureBulkOps.length > 0) {
        bulkPromises.push(
            Lecture.bulkWrite(lectureBulkOps, { 
                session, 
                ordered: false,
                writeConcern: { w: 1 }
            })
